...
public final class GeoJsonParser {
  ...
  private List<Feature> parseFeatures(JsonParser parser) {
    try {
      ...
      String type = node.path("type").getTextValue();
      if (type.equals("FeatureCollection")) {
        ArrayNode jsonFeatures = (ArrayNode) node.path("features");
        return parseFeatures(jsonFeatures);
      } else if (type.equals("GeometryCollection")) {
        ArrayNode jsonFeatures = (ArrayNode) node.path("geometries");
  ...
  private List<Feature> parseFeatures(ArrayNode jsonFeatures) {
    ...
    for (JsonNode jsonFeature : jsonFeatures) {
      String type = jsonFeature.path("type").getTextValue();
      if (!"Feature".equals(type)) {
        continue;
      }
      Geometry g = parseGeometry(jsonFeature.path("geometry"));
      if (outSR != null && outSR.getID() != 4326) {
        g = GeometryEngine.project(g, inSR, outSR);
      }
      Map<String, Object> attributes = parseProperties(jsonFeature.path("properties"));
  ...
  private Geometry parseGeometry(JsonNode node) {
    GeometryType type = GeometryType.fromString(node.path("type").getTextValue());
    return parseCoordinates(type, node.path("coordinates"));
  }
